...
class SensorDependencyNetwork:
    ...
    def build_network(self, data: pd.DataFrame) -> nx.DiGraph:
        ...
        logger.info(f"Network created with {len(self.network.nodes)} nodes and {len(self.network.edges)} edges")
        return self.network
    ...
    def identify_sensor_roles(self) -> Dict:
        ...
        high_threshold = np.percentile(list(in_degree.values()) + list(out_degree.values()), 75)
        low_threshold = np.percentile(list(in_degree.values()) + list(out_degree.values()), 25)
    ...
    def find_critical_sensors(self) -> Dict:
        ...
        # Sort by criticality
        sorted_sensors = sorted(criticality_scores.items(), 
    ...
    def plot_network(self, figsize: Tuple[int, int] = (12, 8), 
        ...
        node_sizes = [centrality[node] * node_size_factor + 100 for node in self.network.nodes()]
        
        # Calculate edge weights for visualization
        edge_weights = [self.network[u][v]['weight'] / 10 for u, v in self.network.edges()]
