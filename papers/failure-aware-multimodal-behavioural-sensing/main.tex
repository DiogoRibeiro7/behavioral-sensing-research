...
remaining claims rest, did not function. The third of these is the most damaging, because
abstention is what the other claims rest on: a system that cannot identify its
own insufficient evidence cannot be trusted to degrade safely, whatever its
accuracy.
...
fails rather than only that it does. The frozen confirmatory simulation has now
been executed and is reported separately; the frozen external cohort remains
unscored, and it is the abstention behaviour, not the accuracy figures, that
most needs that result. On the present evidence the framework does not yet
deliver the property it was built for: a monitoring system whose uncertainty
degrades in step with its evidence.
