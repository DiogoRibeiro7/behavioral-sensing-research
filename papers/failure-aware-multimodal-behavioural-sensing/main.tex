...
The software release used by the frozen experiment satisfied the experiment smoke path before the freeze, and later documentation or validation changes do not move the pinned experiment revision. The confirmatory run has since been executed and accepted under those rules, and its values are reported in the companion manuscript. One outcome from it is used here, its abstention rate under increasing missingness, which we report alongside the separate real-data analysis of \cref{sec:abstentionfailure}.
...
unscored. That cohort's pre-specified primary outcome is the paired
balanced-accuracy difference between candidate and baseline, with abstention
among its secondary outcomes, and that ordering stands; abstention is simply
where the present evidence leaves the largest open question. On that evidence,
the framework does not yet deliver the property it was built for: a monitoring
system whose uncertainty degrades in step with its evidence.
