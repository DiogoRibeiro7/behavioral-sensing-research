...
* Any real deployment must re-derive its emission parameters from its own
  data. The defaults in :mod:`sensor_modeling.fusion.defaults` are documented
...
Snapshots are not versioned
    :meth:`snapshot` output is checked against the current state ontology and
...
:class:`~sensor_modeling.observations.Observation` interface would defeat it.
