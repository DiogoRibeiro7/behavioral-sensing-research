...
| --- | --- | --- | --- |
| `state_frequency` | `StateFrequencyBaseline` | training labels only | training state frequencies, identical for every row |
...
| `tree` | `TreeBaseline` | every permitted column | leaf frequencies of one depth-limited tree |
...
| Baseline | Settings |
| --- | --- |
| `state_frequency` | no smoothing |
| `persistence` | inner `logistic` with the settings below |
| `logistic` | `C = 1.0`, L-BFGS, `max_iter = 1000`, features standardised on training rows, hour one-hot encoded |
| `tree` | `max_depth = 6`, `min_samples_leaf = 20`, hour as an ordinal column |
...
| Log loss, Brier score | the no-information reference: what a model scores without using any row | meaningful; first-timestamp rows carry the training frequencies | meaningful | meaningful, but log loss is sensitive to leaves that give the true state probability zero |
...
A state with no training rows gets probability zero from every baseline and is
never predicted. On a held-out household where that state occurs, each such
row contributes the metric floor to log loss, `-log(1e-12) ≈ 27.6`. When
reading log loss, first check which states the training households contain.
