...
`SensorHealthMonitor` maintains a verdict per
sensor and emits it as an evidence weight in `[0, 1]`.

| Status | Meaning |
| --- | --- |
| `HEALTHY` | Reporting as declared, with plausible values. |
| `DEGRADED` | Still reporting, with reduced measurement quality. |
| `DROPOUT` / `MISSING` | Silent long enough to suspect, then to assume, a fault. |
| `STUCK` | Unchanging value where variation is expected. |
| `DRIFTING` | Calibration has shifted relative to the sensor's own history. |
| `OUT_OF_RANGE` | Values outside the declared plausible range. |
| `UNKNOWN` | Not enough evidence to judge. The honest default, not a failure. |
...
`MultimodalBayesFilter` maintains
...
| Model | Used for |
| --- | --- |
| `PoissonEventEmission` | Event streams. Over an interval `T` with `n` activations, a state with rate `lambda` scores `n log(lambda) - lambda T`. |
| `GaussianEmission` | Sampled scalars such as wearable activity magnitude. |
| `BernoulliEmission` | Persisting binary states such as bed occupancy. Uses only the most recent reading, since repeated reports of an unchanged state are not repeated independent measurements of it. |
| `BetaEmission` | Probability-valued derived features such as a radar presence probability, kept on its natural support rather than thresholded. |
...
`default_emissions` derives models from the
...
`ResidentContextEstimator` maintains a
...
before anyone notices. `baseline` treats behaviour as a
slowly moving distribution and distinguishes the reasons a day can look
unusual:

| Verdict | Meaning |
| --- | --- |
| `ORDINARY` | Within the personal band. Not a finding. |
| `TEMPORARY_DISTURBANCE` | Unusual days that have already reverted. |
| `PERSISTENT_CHANGE` | A shift that has held past `persistence_days`. |
| `GRADUAL_DRIFT` | A slow monotone trend rather than a step. |
| `ABRUPT_CHANGE` | A step located by PELT change-point detection. |
| `INSUFFICIENT_DATA` | Not enough well-observed days to say anything. |
...
`interop` exports the pipeline's output in a FHIR-style
...
| Kind | Exported as |
| --- | --- |
| Measured observation | `Observation`, `status: final`, provenance `measured`. |
| Derived feature | `Observation`, `status: final`, provenance `derived-feature`, carrying the upstream device's own confidence. |
| Inferred state | `Observation`, `status: preliminary`, provenance `inferred`, an explicit `method`, the **whole posterior** as components, and `derivedFrom` listing the contributing sensors. |
| Algorithmic alert | `DetectedIssue` -- never an `Observation`, because an alert is a judgement rather than a record of anything observed. |

Every resource carries a provenance extension recording how it was produced.
`summarise_provenance` counts them, and
`measured_only` returns just the genuine
...
`privacy` provides pseudonymisation and redaction
