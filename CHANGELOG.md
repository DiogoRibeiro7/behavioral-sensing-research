...
- Added the first evaluation against a real recording. On CASAS `hh103`, 57 days, with nothing refitted, balanced accuracy was 0.349 against 0.816 on the simulator and calibration error 0.299 against 0.084. States with a distinctive room-and-rate signature held up, while those needing evidence that the resident is present but still collapsed: where the resident was genuinely inactive at home the pipeline reported `away` in 53% of cases, and abstention fired on 2.1% of steps while the model was wrong more often than right. Recorded in `docs/real_data.md` and `docs/limitations.md`.
